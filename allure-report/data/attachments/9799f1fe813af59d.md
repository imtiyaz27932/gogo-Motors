...
Locator: locator('p').filter({ hasText: /Clean & Prep|Cut & Shape|Apply & Smooth/ }).first()
Expected: 3
Received: 1
Call log:
  - expect.toHaveCount with timeout 45000ms
  - waiting for locator('p').filter({ hasText: /Clean & Prep|Cut & Shape|Apply & Smooth/ }).first()
    48 × locator resolved to 1 element
       - unexpected value "1"
