...
Locator: getByRole('paragraph').filter({ hasText: /^Benefits of tinting$/ }).first()
Expected: visible
Received: <element(s) not found>
Call log:
  - expect.toBeVisible with timeout 45000ms
  - waiting for getByRole('paragraph').filter({ hasText: /^Benefits of tinting$/ }).first()

    at CarTinting.verifyBenefitsOfTintingSection (D:\gogomotor\pages\CarTinting\tinting.ts:515:42)
