...
Locator: locator('p.text-\\[32px\\].font-semibold').filter({ hasText: 'Perfect Fit, Every Time' }).locator('xpath=ancestor::div[contains(@class,"rounded-[26px]")]').locator('p.text-\\[15px\\].font-semibold')
Expected: 4
Received: 3
Call log:
  - expect.toHaveCount with timeout 45000ms
  - waiting for locator('p.text-\\[32px\\].font-semibold').filter({ hasText: 'Perfect Fit, Every Time' }).locator('xpath=ancestor::div[contains(@class,"rounded-[26px]")]').locator('p.text-\\[15px\\].font-semibold')
    48 × locator resolved to 3 elements
       - unexpected value "3"

    at CarTinting.verifyPerfectFitSection (D:\gogomotor\pages\CarTinting\tinting.ts:515:37)
...
> 515 |     await expect(this.stepTitles()).toHaveCount(4);
      |                                     ^ Error: Timed out 45000ms waiting for expect(locator).toHaveCount(expected)
  516 |
  517 |     Logger.info('Validating presence of all 3 step descriptions');
  518 |     await expect(this.stepDescriptions()).toHaveCount(4);
  519 |
  520 |   }
  521 |
